...
A la hora de afrontar un proyecto que simule cierto comportamiento \emph{humano}, debemos usar técnicas de Inteligencia Artificial, en busca de herramientas, técnicas y metodologías que nos ayuden a afrontar este
...
El proceso de construcción de un sistema experto se denomina ingeniería del conocimiento. Se basa en una serie de fases que se iteran hasta conseguir un sistema adecuado (figura~\vref{des_sistexp}). En una primera etapa el ingeniero de conocimiento tiene una entrevista con el experto, en la que intenta obtener sus conocimientos. Después los implementa en el sistema. Y, por último, el experto evalúa el sistema. Si la evaluación no es del todo satisfactoria, se repite el proceso para refinar el resultado. \\
...
\begin{description}
    ...
    \item[Interfaz de usuario] Es la interacción entre el sistema experto y el usuario, y se realiza mediante el lenguaje natural.
\end{description}


\subsection{Sistemas expertos basados en reglas}
Los sistemas expertos basados en reglas (sistemas expertos a partir de
...
\begin{description}
\item[Interfaz de usuario:] permite la interacción con el sistema. La
  interacción la puede realizar tanto una persona como otro sistema
  que desee hacer uso del sistema experto.
\item[Medio de explicación:] explica al usuario el razonamiento del
  sistema. No siempre está presente en todos los sistemas.
\item[Memoria activa:] es una base de datos con la información que
  usarán las reglas.
\item[Mecanismo de inferencia:] comprueba las reglas que satisface la
  memoria activa y ejecuta la de mayor prioridad. La agenda es el
  conjunto de reglas satisfechas en un momento dado.
\item[Base de conocimiento: ] contiene los conocimientos que el
  sistema ha aprendido en forma de reglas.
\item[Medio para la adquisición de conocimiento:] permite al usuario
  introducir nuevo conocimiento en el sistema sin tener que mediar el
  ingeniero de conocimiento. El sistema suele recibir ejemplos para
  aprender de ellos.
\end{description}
...
almacena. En concreto se especifican los atributos (del inglés
\emph{slots}) de cada objeto. Y los objetos almacenan información
con la estructura indicada en las plantillas.


\subsubsection{Base de conocimiento}

La base de conocimiento (también llamada sistema de producción) de un
...
activación y la ejecución de una regla. La activación indica que la
...
(llamado regla de aproximación a la búsqueda de hechos) sería recorrer
circularmente de modo indefinido el conjunto de reglas, buscando
reglas que se satisfagan y ejecutándolas. El problema es su enorme
ineficiencia. Tiene una complejidad computacional del orden de
$O(R*F^P)$, siendo \emph{R} el número de reglas, \emph{P} la media de
comprobaciones por el \emph{L.H.S.} de cada regla y \emph{F} la
cantidad de reglas de la base de conocimiento. Evidentemente, este
orden no es deseable para sistemas grandes, pues se dispara al incrementarse las
comprobaciones por regla.
